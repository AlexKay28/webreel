...
<sub>The GIF above is regenerated by the [`demo` workflow](.github/workflows/demo.yml) — trigger it from the Actions tab, or run [`scripts/generate_demo.py`](scripts/generate_demo.py) locally.</sub>

**Prefer scripted tours?** Three example scenarios live in [`docs/scenarios/`](docs/scenarios/) (landing page, form submission, ARIA tab widget). They run against the built-in fixture site:

```bash
clickcast run docs/scenarios/spa.yml --var base_url=http://127.0.0.1:8000
```

**Wiring this into an AI agent?** See [`docs/ai-integration.md`](docs/ai-integration.md) for the two-line loop, and [`docs/feedback-schema.md`](docs/feedback-schema.md) for the JSON sidecar contract.
